...
//		int[] arr = {3,3, 5,6,6, 7, 10, 11,11,11,11,11,11,11};
		int[] arr = { 34, 42, 20, 18, 26, 29, 23, 47, 3, 15, 45, 27, 14, 29, 46, 4, 39, 4, 40, 12, 16, 18, 19, 17, 11,
				25, 19, 30, 49, 10, 14, 41, 24, 20, 46, 37, 43, 36, 4, 3, 10, 18, 12, 39, 29 }; // expected = 9
...
//	            }
	...
	public boolean isCircularSentence(String sentence) {
		...
		for (int i = 0; i < wordArr.length - 1; i++) {
			...
			String nextWord = wordArr[i + 1];
			if (word.charAt(0) != nextWord.charAt(nextWord.length() - 1)) {
				return false;
			}
		}
		if (wordArr[0].charAt(0) != wordArr[size - 1].charAt(wordArr[size - 1].length() - 1)) {
			return false;
		}
		return true;
	}
